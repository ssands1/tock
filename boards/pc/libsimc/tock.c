...
typedef enum { COMMAND, SUBSCRIBE, ALLOW } Syscall_t;
...
int make_call(Syscall_t sys, uint32_t driver, uint32_t subdriver, size_t arg3, 
                                                                  size_t arg4) {
...
int allow(uint32_t driver, uint32_t subdriver, void *ptr, size_t size) {
    return make_call(ALLOW, driver, subdriver, (size_t) ptr, size);
}
